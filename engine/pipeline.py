...
async def build_video(project_root: Path, episode_name: str) -> Path:
    ...
    for scene in plan.scenes:
        ...
        prepared = asset_manager.prepare(scene.asset, shot.focus_x, shot.focus_y)
        overlay = None
        ...
        clips.append(renderer.render_scene(scene, prepared, overlay))

    print("[timeline] compondo cortes, crossfades e legendas...")
    timeline_video = renderer.compose_timeline(clips, plan, captions_path)
    print("[final] adicionando voz e mixagem...")
    output = renderer.mux_audio(timeline_video, audio, f"{episode.story.slug}.mp4")
